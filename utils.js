...
Number.implement({
  ...
  toFileSize: function(numdec, unit) {
    ...
    return (
      size.toFixedNR(typeOf(numdec) == "number" ? numdec : 1) + " " + sz[unit]
    );
  },
  ...
  toTimeDelta: function() {
    ...
    s = div % 60;
    if (y > 0) {
      output = L_("TIME_YEARS_WEEKS")
        .replace(/%d/, y)
        .replace(/%d/, w);
    } else if (w > 0) {
      output = L_("TIME_WEEKS_DAYS")
        .replace(/%d/, w)
        .replace(/%d/, d);
    } else if (d > 0) {
      output = L_("TIME_DAYS_HOURS")
        .replace(/%d/, d)
        .replace(/%d/, h);
    } else if (h > 0) {
      output = L_("TIME_HOURS_MINS")
        .replace(/%d/, h)
        .replace(/%d/, m);
    } else if (m > 0) {
      output = L_("TIME_MINS_SECS")
        .replace(/%d/, m)
        .replace(/%d/, s);
    } else {
...
Element.implement({
  ...
  addClasses: function() {
    var l = arguments.length,
      clear = false,
      hasChanged = false;
    if (typeOf(arguments[l - 1]) == "boolean") clear = arguments[--l];
    ...
    if (hasChanged) this.className = cls.clean();
    return this;
  }
});
...
Event.implement({
  isRightClick: function() {
    return !!(
      this.rightClick ||
      (this.control && this.event.button === 0 && Browser.Platform.mac)
    );
  },
  ...
  withinScroll: function(ele) {
    ...
    return (
      pos.x <= x &&
      x <= pos.x + ele.clientWidth &&
      pos.y <= y &&
      y <= pos.y + ele.clientHeight
    );
  }
});
