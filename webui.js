...
(function() {
  ...
  var urlBase = window.location.pathname.substr(
    0,
    window.location.pathname.indexOf("/gui")
  );
  ...
  */
  ...
  var utWebUI = {
    ...
    init: function() {
      ...
      this.trtColDoneIdx = this.trtColDefs
        .map(function(item) {
          return item[0] == "done";
        })
        .indexOf(true);
      this.trtColStatusIdx = this.trtColDefs
        .map(function(item) {
          return item[0] == "status";
        })
        .indexOf(true);
      this.flsColPrioIdx = this.flsColDefs
        .map(function(item) {
          return item[0] == "prio";
        })
        .indexOf(true);
      ...
      this.trtColDefs.each(function(item, index) {
        this.trtColToggle(index, item[3], true);
      }, this);
      this.prsColDefs.each(function(item, index) {
        this.prsColToggle(index, item[3], true);
      }, this);
      this.flsColDefs.each(function(item, index) {
        this.flsColToggle(index, item[3], true);
      }, this);
      this.fdColDefs.each(function(item, index) {
        this.fdColToggle(index, item[3], true);
      }, this);
    ...
    beginPeriodicUpdate: function(delay) {
      ...
      this.config.updateInterval = delay = delay.max(
        this.limits.minUpdateInterval
      );
        ...
        function(fid) {
          new IFrame({
            ...
            src:
              ...
              fid +
              "&disposition=" +
              (streaming ? "INLINE" : "ATTACHMENT") +
              "&service=DOWNLOAD&qos=0",
            styles: {
              display: "none",
              height: 0,
              width: 0
    ...
    request: function(qs, fn, async, fails) {
      ...
      if (window.getraptor) {
        ...
        return getraptor().post_raw(
          qs,
          {},
          fn ? fn.bind(self) : function(resp) {},
          fails,
          { async: async }
        );
      }
      ...
      var req = function() {
        try {
          new Request.JSON({
            url:
              guiBase + "?token=" + self.TOKEN + "&" + qs + "&t=" + Date.now(),
            method: "get",
            ...
            onFailure: function() {
              ...
              self.TOKEN = "";
              self.request.delay(delay * 1000, self, [
                qs,
                function(json) {
                  if (fails[0]) {
                    fails[0] = 0;
    ...
    perform: function(action) {
      ...
      this.getList(
        "action=" + action + "&hash=" + hashes.join("&hash="),
        function() {
          this.updateToolbar();
        }.bind(this)
      );
    ...
    getHashes: function(act, all) {
      ...
      while (len--) {
        ...
        switch (act) {
          ...
          case "queuebottom":
            key = {
              qnum: this.torrents[key][CONST.TORRENT_QUEUE_POSITION],
              hash: key
            };
        ...
        switch (act) {
          case "queuedown":
            limsetter = function(item) {
              if (item.qnum == queueLimMax) --queueLimMax;
            };
          ...
          case "queueup":
            limsetter = function(item) {
              if (item.qnum == queueLimMin) ++queueLimMin;
            };
            qfilter = function(item) {
          ...
          case "queuebottom":
            ...
            limsetter = function(_, i, list) {
              ...
              if (qnum == queueLimMax) --queueLimMax;
              if (qnum < qmin) qmin = qnum;
            };
            qfilter = function(item) {
          ...
          case "queuetop":
            ...
            limsetter = function(_, i, list) {
              var qnum = list[hashcount - i - 1].qnum;
              if (qnum == queueLimMin) ++queueLimMin;
              if (qmax < qnum) qmax = qnum;
            };
            qfilter = function(item) {
    ...
    removeDefault: function(shift) {
      this.remove(
        (this.settings["gui.default_del_action"] || 0) | (shift ? 2 : 0)
      );
    },
    ...
    rssUpdate: function(param, fn) {
      {
        // TODO: Remove this once backend support is stable (requires 3.0+)
        if (
          window.raptor === undefined &&
          undefined === this.settings["webui.uconnect_enable"]
        )
          return;
      }
    ...
    rssfilterUpdate: function(param, fn) {
      ...
      Array.from(param.id).each(function(id) {
        ...
        if (param) {
          ...
          if (undefined !== id) qs += "&filter-id=" + (parseInt(id, 10) || -1);
    ...
    getList: function(qs, fn) {
      ...
      if (qs != "") qs += "&";
      this.request(
        qs + "list=1&cid=" + this.cacheID + "&getmsg=1",
        function(json) {
          this.loadList(json);
          if (fn) fn(json);
        }.bind(this)
    ...
    getStatusInfo: function(state, done) {
      ...
      if (state & CONST.STATE_PAUSED) {
        ...
        res = [
          "Status_Paused",
          state & CONST.STATE_CHECKING
            ? L_("OV_FL_CHECKED").replace(/%:\.1d%/, (done / 10).toFixedNR(1))
            : L_("OV_FL_PAUSED")
        ];
      } else if (state & CONST.STATE_STARTED) {
        // started, seeding or leeching
        res =
          done == 1000
            ? ["Status_Up", L_("OV_FL_SEEDING")]
            : ["Status_Down", L_("OV_FL_DOWNLOADING")];
      ...
      } else if (state & CONST.STATE_QUEUED) {
        // queued
        res =
          done == 1000
            ? ["Status_Queued_Up", L_("OV_FL_QUEUED_SEED")]
            : ["Status_Queued_Down", L_("OV_FL_QUEUED")];
      } else if (done == 1000) {
        // finished
      ...
      function extractLists(
        ...
        if (!has(json, fullListName)) {
          if (!has(json, changedListName)) {
            ...
            extracted.hasChanged =
              extracted[fullListName].length +
                extracted[removedListName].length >
              0;
          }
        } else {
        ...
        this.trtTable.keepScroll(
          function() {
            ...
            deltaLists.torrents.each(function(item) {
              ...
              var statinfo = this.getStatusInfo(
                item[CONST.TORRENT_STATUS],
                item[CONST.TORRENT_PROGRESS]
              );
              ...
              if (has(this.torrents, hash)) {
                ...
                row.each(function(v, k) {
                  if (v != rdata.data[k]) {
                    ...
                    if ("done" == this.trtColDefs[k][0]) {
                      ...
                      ret =
                        this.trtTable.updateCell(
                          hash,
                          this.trtColStatusIdx,
                          row
                        ) || ret;
                    }
                  }
                ...
                this.trtTable.addRow(
                  row,
                  hash,
                  statinfo[0],
                  !this.torrentIsVisible(hash)
                );
                ret = true;
              }
            ...
            this.trtTable.requiresRefresh =
              sortedColChanged || this.trtTable.requiresRefresh;

            // Handle removed items
            var clear = false;

            if (window.utweb === undefined) {
              // torGroups does not get setup properly in remote
              deltaLists.torrentm.each(function(key) {
                Object.each(
                  this.torGroups[key].cat,
                  function(_, cat) {
                    --this.categories[cat];
                  },
                  this
            ...
            var queueMax = -1,
              q = CONST.TORRENT_QUEUE_POSITION;
          ...
          filterList.grab(
            new Element("li", {
              ...
              new Element("input", {
                ...
                checked: !!(
                  filter[CONST.RSSFILTER_FLAGS] & CONST.RSSFILTERFLAG_ENABLE
                )
              }),
              "top"
            )
    ...
    loadRSSFilter: function(filterId) {
      ...
      _loadComboboxStrings(
        "rssfilter_feed",
        feeds,
        [filter[CONST.RSSFILTER_FEED], -1].pick()
      );
      ...
      $("rssfilter_save_in").set(
        "value",
        filter[CONST.RSSFILTER_DIRECTORY] || ""
      );
      $("rssfilter_episode").set(
        "value",
        filter[CONST.RSSFILTER_EPISODE_FILTER_STR] || ""
      );
      $("rssfilter_episode_enable").checked = !!filter[
        CONST.RSSFILTER_EPISODE_FILTER
      ];
      $("rssfilter_quality").set(
        "value",
        this.rssfilterQualityText(filter[CONST.RSSFILTER_QUALITY])
      );
      $("rssfilter_orig_name").checked = !!(
        filter[CONST.RSSFILTER_FLAGS] & CONST.RSSFILTERFLAG_ORIG_NAME
      );
      $("rssfilter_add_stopped").checked = !!(
        filter[CONST.RSSFILTER_FLAGS] & CONST.RSSFILTERFLAG_ADD_STOPPED
      );
      $("rssfilter_prio").checked = !!(
        filter[CONST.RSSFILTER_FLAGS] & CONST.RSSFILTERFLAG_HIGH_PRIORITY
      );
      $("rssfilter_smart_ep").checked = !!(
        filter[CONST.RSSFILTER_FLAGS] & CONST.RSSFILTERFLAG_SMART_EP_FILTER
      );
      $("rssfilter_min_interval").set(
        "value",
        filter[CONST.RSSFILTER_POSTPONE_MODE] || 0
      );
    ...
    update: function(listcb) {
      ...
      if (typeof DialogManager !== "undefined") {
        if (
          DialogManager.showing.contains("Settings") &&
          "dlgSettings-TransferCap" == this.stpanes.active
        ) {
    ...
    loadLabels: function(labels) {
      var labelList = $("mainCatList-labels"),
        temp = {};
      labels.each(function(lbl, idx) {
        var label = lbl[0],
          labelId = "lbl_" + encodeID(label),
          count = lbl[1],
          li = null;
        if ((li = $(labelId))) {
        ...
        if (has(this.labels, label)) delete this.labels[label];
        temp[label] = count;
      }, this);
    ...
    torrentIsVisible: function(hash) {
      ...
      if (
        visible &&
        (actCat["cat_nlb"] || Object.some(actLbl, Function.from(true)))
      ) {
        visible =
          visible &&
          ((actCat["cat_nlb"] && group.cat["cat_nlb"]) ||
            Object.some(actLbl, function(_, lbl) {
              return group.lbl[lbl];
            }));
      }
    ...
    getTorGroups: function(tor) {
      ...
      if (
        tor[CONST.TORRENT_DOWNSPEED] >
          (this.settings["queue.slow_dl_threshold"] || 103) ||
        tor[CONST.TORRENT_UPSPEED] >
          (this.settings["queue.slow_ul_threshold"] || 103)
      ) {
    ...
    newLabel: function() {
      ...
      if (this.trtTable.selectedRows.length == 1)
        tmpl = this.torrents[this.trtTable.selectedRows[0]][
          CONST.TORRENT_LABEL
        ];
    ...
    updateLabels: function() {
      ["cat_all", "cat_dls", "cat_com", "cat_act", "cat_iac", "cat_nlb"].each(
        function(cat) {
    ...
    catListClick: function(ev, listId) {
      ...
      var prevSelected =
        activeGroupCount > 1 &&
        Object.some(this.config.activeTorGroups, function(type) {
          return eleId in type;
        });

      if (
        (Browser.Platform.mac && ev.meta) ||
        (!Browser.Platform.mac && ev.control)
      ) {
        if (ev.isRightClick()) {
          prevSelected = false;
        }
        ...
        if (!(ev.isRightClick() && prevSelected)) {
          ...
          Object.each(
            this.defConfig.activeTorGroups,
            function(_, type) {
    ...
    refreshSelectedTorGroups: function() {
      ...
      this.__refreshSelectedTorGroups_activeTorGroups__ = Object.merge(
        {},
        this.config.activeTorGroups
      );
      if (!oldGroups) {
      ...
      for (var hash in this.torrents) {
        var changed =
          !!this.trtTable.rowData[hash].hidden ===
          !!this.torrentIsVisible(hash);
        if (changed) {
          activeChanged = true;
    ...
    feedListClick: function(ev) {
      ...
      if (element) {
        ...
        var prevSelected =
          activeFeedCount > 1 && element.id in this.config.activeRssFeeds;
      ...
      if (!isGuest && ev.isRightClick()) {
        ...
        if (element) {
          var feedIds =
            "rssfeed_all" in this.config.activeRssFeeds
              ? Object.keys(this.rssfeeds)
              : Object.keys(this.config.activeRssFeeds).map(function(feed) {
                  return feed.replace(/^rssfeed_/, "");
                });
          ...
          if (feedIds.length > 0) {
            ...
            if (feedIds.length === 1) {
              menuItems.push([
                L_("DLG_RSSDOWNLOADER_19"),
                this.showAddEditRSSFeed.bind(this, feedIds[0])
              ]);
            }
            ...
            menuItems = menuItems.concat([
              [
                ...
                this.rssUpdate.bind(
                  this,
                  { id: feedIds, enabled: !feedIsEnabled },
                  null
                )
              ],
              [
                L_("DLG_RSSDOWNLOADER_22"),
                this.rssUpdate.bind(this, { id: feedIds, update: 1 }, null)
              ],
              [
                ...
                function(feedIds) {
                  ...
                  DialogManager.popup({
                    ...
                    message:
                      feedIds.length > 1
                        ? L_("DLG_RSSDOWNLOADER_35").replace(
                            /%d/,
                            feedIds.length
                          )
                        : L_("DLG_RSSDOWNLOADER_36").replace(
                            /%s/,
                            this.rssfeeds[feedIds][CONST.RSSFEED_URL].split(
                              "|"
                            )[0]
                          ),
                    buttons: [
                      {
                        text: L_("DLG_BTN_YES"),
                        focus: true,
                        click: this.rssRemove.bind(this, feedIds)
                      },
                      { text: L_("DLG_BTN_NO") }
                    ]
                  });
    ...
    refreshSelectedRSSFeeds: function() {
      ...
      if (oldFeeds) {
        ...
        if (!selCount) {
          deltaFeeds = this.config.activeRssFeeds = Object.merge(
            {},
            this.defConfig.activeRssFeeds
          );
        }
      }

      this.__refreshSelectedRSSFeeds_activeRssFeeds__ = Object.merge(
        {},
        this.config.activeRssFeeds
      );
      if (!oldFeeds) {
      ...
      this.rssfdTable.keepScroll(
        function() {
          ...
          Object.each(
            ...
            function(feed, id) {
              if (
                !activeFeeds["rssfeed_all"] &&
                !("rssfeed_" + id in activeFeeds)
              )
                return;
    ...
    rssfilterListClick: function(ev) {
      ...
      if (!isGuest && ev.isRightClick()) {
        ...
        if (element) {
          ...
          menuItems.push([
            L_("DLG_RSSDOWNLOADER_28"),
            function(filterId) {
              // RSSTODO: Move this elsewhere
              DialogManager.popup({
                title: "Remove RSS Filter(s)", // TODO: Localize
                icon: "dlgIcon-Delete",
                message: L_("OV_CONFIRM_DELETE_RSSFILTER").replace(
                  /%s/,
                  this.rssfilters[filterId][CONST.RSSFILTER_NAME]
                ),
                buttons: [
                  {
                    text: L_("DLG_BTN_YES"),
                    focus: true,
                    click: this.rssfilterRemove.bind(this, filterId)
                  },
                  { text: L_("DLG_BTN_NO") }
                ]
              });
    ...
    rssfilterEdited: function(ev) {
      ...
      if (!filter) {
        ...
        this.rssfilters.EDIT = filter = Array.clone(
          this.rssfilters[this.rssfilterId.replace(/^rssfilter_/, "")] || []
        );
      }
      ...
      if (ev) {
        ...
        switch (ev.target.id) {
          case "rssfilter_name":
            idx = CONST.RSSFILTER_NAME;
            break;
          case "rssfilter_filter":
            idx = CONST.RSSFILTER_FILTER;
            break;
          case "rssfilter_not":
            idx = CONST.RSSFILTER_NOT_FILTER;
            break;
          case "rssfilter_save_in":
            idx = CONST.RSSFILTER_DIRECTORY;
            break;
          case "rssfilter_label":
            idx = CONST.RSSFILTER_LABEL;
            break;
          case "rssfilter_feed":
            idx = CONST.RSSFILTER_FEED;
            break;
          case "rssfilter_min_interval":
            idx = CONST.RSSFILTER_POSTPONE_MODE;
            break;

          case "rssfilter_episode":
            idx = CONST.RSSFILTER_EPISODE_FILTER_STR;
            break;
    ...
    rssfilterEditApply: function() {
      var oldFilter = this.rssfilters[
        this.rssfilterId.replace(/^rssfilter_/, "")
      ];
      if (!oldFilter) return;
    ...
    rssfilterQualityClick: function(ev) {
      ...
      var menuItems = [
        [
          CMENU_CHECK,
          L_("DLG_RSSDOWNLOADER_29"),
          qualSelect(CONST.RSSITEMQUALITY_ALL)
        ],
        [CMENU_SEP]
      ];
    ...
    getDirectoryList: function(forceload, callback) {
      ...
      if (
        forceload ||
        !this.dirlist._TIME_ ||
        now - this.dirlist._TIME_ > this.limits.minDirListCache * 1000
      ) {
        this.request(
          "action=list-dirs",
          function(json) {
    ...
    loadDirectoryList: function() {
      ...
      var items = list.map(function(dir) {
        return (
          "[" +
          parseInt(dir.available, 10).toFileSize(2, 2) +
          " free] " +
          dir.path
        );
      });
      ...
      if (eleList) eleList.setStyle("display", dispStyle);
      if (eleListDiv) eleListDiv.setStyle("display", dispStyle);
      if (eleURLList) eleURLList.setStyle("display", dispStyle);
      if (eleURLListDiv) eleURLListDiv.setStyle("display", dispStyle);
    },
    ...
    getTransferHistory: function(forceload) {
      ...
      if (
        forceload ||
        !this.xferhist._TIME_ ||
        now - this.xferhist._TIME_ > this.limits.minXferHistCache * 1000
      ) {
        this.request(
          "action=getxferhist",
          function(json) {
    ...
    loadTransferHistory: function() {
      ...
      $("history_period").set(
        "text",
        L_("DLG_SETTINGS_7_TRANSFERCAP_11").replace(/%d/, period)
      );
    },
    ...
    resetTransferHistory: function() {
      ...
      this.request("action=resetxferhist", function() {
    ...
    addSettings: function(json, fn) {
      var loadCookie = function(newcookie) {
        ...
        cookie.activeRssFeeds =
          newcookie.activeRssFeeds || this.defConfig.activeRssFeeds || {};
        cookie.activeTorGroups =
          newcookie.activeTorGroups || this.defConfig.activeTorGroups || {};
      ...
      if (isGuest) {
        ...
        for (var i = 0, j = json.settings.length; i < j; i++) {
          ...
          switch (key) {
            ...
            case "gui.alternate_color":
              if (window.utweb === undefined) {
                this.tableUseAltColor(val);
              }
              break;
            case "gui.graph_legend":
              if (window.utweb === undefined) {
                this.spdGraph.showLegend(val);
              }
              break;
            case "gui.graphic_progress":
              if (window.utweb === undefined) {
                this.tableUseProgressBar(val);
              }
              break;
            case "gui.log_date":
              if (window.utweb === undefined) {
                Logger.setLogDate(val);
              }
              break;

            case "bt.transp_disposition":
              $("enable_bw_management").checked = !!(val & CONST.TRANSDISP_UTP);
              break;
          }
        ...
        {
          // TODO: Remove this once backend support is stable (requires 3.0+)
          this.settings["sched_table"] = [
            this.settings["sched_table"],
            "033000330020000000000000300303003222000000000000000303003020000000000000033003003111010010100101000303003101011010100111300303003101010110100001033020330111010010110111"
          ].pick();
          this.settings["search_list_sel"] = [
            this.settings["search_list_sel"],
            0
          ].pick();
          this.settings["search_list"] = [
            this.settings["search_list"],
      ...
      if (!(this.config.lang in LANG_LIST)) {
        ...
        var useLang = (navigator.language
          ? navigator.language
        ...
        if (useLang && useLang in LANG_LIST) this.config.lang = useLang;
        else this.config.lang = this.defConfig.lang || "en";
      }
      if (window.utweb) return;
    ...
    loadSettings: function() {
      ...
      if (!this.config.showToolbar && !isGuest) $("mainToolbar").hide();
    ...
    setSettings: function() {
      ...
      if (
        undefined != value &&
        !!this.settings["gui.graphic_progress"] != value
      ) {
      ...
      for (var key in this.settings) {
        ...
        if (
          key === "webui.uconnect_enable" ||
          key === "webui.uconnect_username"
        )
          continue;
        ...
        if (v != nv) {
          ...
          if (key == "multi_day_transfer_mode") {
            str +=
              "&s=multi_day_transfer_mode_ul&v=" +
              (nv == 0 ? 1 : 0) +
              "&s=multi_day_transfer_mode_dl&v=" +
              (nv == 1 ? 1 : 0) +
              "&s=multi_day_transfer_mode_uldl&v=" +
              (nv == 2 ? 1 : 0);
            continue;
          }
      ...
      var port = window.location.port
        ...
        : 443;
      var new_port =
        this.settings["webui.enable_listen"] === undefined ||
        this.settings["webui.enable_listen"]
          ? this.settings["webui.port"]
          : this.settings["bind_port"];
    ...
    registerRemote: function() {
      ...
      if (!username_control || !password_control) return;
      var uc_enable = 0;
      var enable_control = $("webui.uconnect_enable");
      if (enable_control) uc_enable = enable_control.checked ? 1 : 0;
      if (0 == uc_enable) {
      ...
      this.sendRemoteRegistrationRequest(
        uc_username,
        uc_password,
        remote_success_callback,
        remote_failure_callback,
        { async: true }
      );
    ...
    saveRemoteEnableStatus: function(username) {
      var str =
        "&s=webui.uconnect_enable&v=1&s=webui.uconnect_username&v=" + username;
      if (username) {
    ...
    saveRemoteSignOutStatus: function() {
      ...
      this.request(
        "action=setsetting" + str,
        this.presentRemoteSignOutSuccessResults,
        true
      );
    },
    ...
    sendRemoteRegistrationRequest: function(
      username,
      password,
      success_cb,
      fail_cb,
    ...
    showRemoteStatus: function(statusCode) {
      ...
      switch (statusCode) {
        ...
        case 9: {
          status_input.css("color", "red");
          status_input.text(
            "Attempting to configure using authentication that doesn't support configuration"
          );
          break;
        }
        case 10: {
    ...
    showAddEditRSSFeed: function(feedId) {
      ...
      $("aerssfd-use_custom_alias")
        .set("checked", use_cust_alias)
        .fireEvent("change");
    ...
    hideSettings: function(load_settings) {
      if (load_settings) utWebUI.loadSettings();
    ...
    searchExecute: function() {
      ...
      if (searchURLs[searchActive]) openURL(searchURLs[searchActive]);
    },
    ...
    searchMenuShow: function(ele) {
      ...
      searchURLs = searchURLs.map(function(item) {
        if (item) return (item.split("|")[0] || "").replace(/ /g, "&nbsp;");
        else return "";
      });
      ...
      Array.each(
        ...
        function(item) {
          if (!item) {
            ...
            if (index == searchActive) ContextMenu.add([CMENU_SEL, item]);
            else ContextMenu.add([item, this.searchMenuSet.bind(this, index)]);
    ...
    generateSpeedList: function(curSpeed, itemCount) {
      ...
      for (
        var i = 1, curscale = (scale - 2).max(0);
        i <= itemCount;
        ++i, curscale += scaleinc
      ) {
        ...
        [
          ...
          [
            ...
            [
              ...
              this.toggleToolbar.bind(this, undefined)
            ],
            [
              L_("MM_OPTIONS_SHOW_DETAIL"),
              this.toggleDetPanel.bind(this, undefined)
            ],
            [
              L_("MM_OPTIONS_SHOW_STATUS"),
              this.toggleStatusBar.bind(this, undefined)
            ],
            [
              L_("MM_OPTIONS_SHOW_CATEGORY"),
              this.toggleCatPanel.bind(this, undefined)
            ],
            [CMENU_SEP],
            [
              L_("MM_OPTIONS_TAB_ICONS"),
              this.toggleDetPanelIcons.bind(this, undefined)
            ]
          ]
        ],
        [
          ...
          [
            [
              L_("MM_HELP_UT_WEBPAGE"),
              openURL.pass(["http://www.utorrent.com/", null])
            ],
            [
              L_("MM_HELP_UT_FORUMS"),
              openURL.pass(["http://forum.utorrent.com/", null])
            ],
            [CMENU_SEP],
            [
              L_("MM_HELP_WEBUI_FEEDBACK"),
              openURL.pass([
    ...
    setSpeedDownload: function(val) {
      // TODO: Generalize settings storage requests
      this.request(
        "action=setsetting&s=max_dl_rate&v=" + val,
        function() {
    ...
    setSpeedUpload: function(val) {
      // TODO: Generalize settings storage requests
      this.request(
        "action=setsetting&s=max_ul_rate&v=" + val,
        function() {
      ...
      return this.statusSpeedMenuShow(
        {
          ...
          list:
            !!this.settings["gui.manual_ratemenu"] &&
            this.settings["gui.dlrate_menu"]
        },
        ev
      );
    },
    ...
    trtDataToRow: function(data) {
      return this.trtColDefs.map(function(item) {
        switch (item[0]) {
          ...
          case "peers":
            return (
              data[CONST.TORRENT_PEERS_CONNECTED] +
              " (" +
              data[CONST.TORRENT_PEERS_SWARM] +
              ")"
            );
          ...
          case "seeds":
            return (
              data[CONST.TORRENT_SEEDS_CONNECTED] +
              " (" +
              data[CONST.TORRENT_SEEDS_SWARM] +
              ")"
            );
          ...
          case "status":
            return [
              data[CONST.TORRENT_STATUS],
              data[CONST.TORRENT_STATUS_MESSAGE]
            ];
    ...
    trtFormatRow: function(values, index) {
      ...
      for (var i = index || 0; i < len; i++) {
        switch (this.trtColDefs[i][0]) {
          ...
          case "eta":
            values[i] =
              values[i] == 0
                ...
                : values[i] == -1
                ? "\u221E"
                : values[i].toTimeDelta();
            break;

          case "ratio":
            values[i] =
              values[i] == -1 ? "\u221E" : (values[i] / 1000).toFixedNR(3);
            break;
          ...
          case "seeds_peers":
            values[i] =
              $chk(values[i]) && values[i] != Number.MAX_VALUE
                ? values[i].toFixedNR(3)
                : "\u221E";
            break;
    ...
    trtSortCustom: function(col, dataX, dataY) {
      ...
      switch (this.trtColDefs[col][0]) {
        case "status":
          var statX = dataX[col][0],
            statY = dataY[col][0];
          ret = (statY & CONST.STATE_ERROR) - (statX & CONST.STATE_ERROR); // errored sorts before unerrored
          if (!ret) {
            ret =
              (statY & CONST.STATE_CHECKING) - (statX & CONST.STATE_CHECKING); // checking sorts before non-checking
            if (!ret) {
              ret =
                (statY & CONST.STATE_STARTED) - (statX & CONST.STATE_STARTED); // started sorts before unstarted
              if (!ret) {
                ret =
                  (statY & CONST.STATE_PAUSED) - (statX & CONST.STATE_PAUSED); // paused sorts before unpaused
                if (!ret) {
    ...
    trtDblClk: function(id) {
      if (!isGuest && this.trtTable.selectedRows.length == 1) {
        ...
        switch (action) {
          ...
          default:
            this.perform(
              tor[CONST.TORRENT_STATUS] &
                (CONST.STATE_STARTED | CONST.STATE_QUEUED)
                ? "stop"
                : "start"
            );
        }
    ...
    showTrtMenu: function(ev, id) {
      ...
      if (
        !this.trtTable.selectedRows.every(function(item) {
          return this.torrents[item][labelIndex] == "";
        }, this)
      ) {
        labelSubMenu.push([
          L_("OV_REMOVE_LABEL"),
      ...
      if (Object.getLength(this.labels) > 0) {
        ...
        $each(
          ...
          function(_, label) {
            if (
              this.trtTable.selectedRows.every(function(item) {
                return this.torrents[item][labelIndex] == label;
              }, this)
            ) {
      ...
      var menuItemsMap = {
        ...
        queueup: [
          L_("ML_QUEUEUP"),
          function(ev) {
            this.queueup(ev.shift);
          }.bind(this)
        ],
        queuedown: [
          L_("ML_QUEUEDOWN"),
          function(ev) {
            this.queuedown(ev.shift);
          }.bind(this)
        ],
        ...
        removeand: [
          ...
          [
            [
              L_("ML_DELETE_TORRENT"),
              this.remove.bind(this, CONST.TOR_REMOVE_TORRENT)
            ],
            [
              L_("ML_DELETE_DATATORRENT"),
              this.remove.bind(this, CONST.TOR_REMOVE_DATATORRENT)
            ],
            [
              L_("ML_DELETE_DATA"),
              this.remove.bind(this, CONST.TOR_REMOVE_DATA)
            ]
          ]
        ],
        recheck: [L_("ML_FORCE_RECHECK"), this.recheck.bind(this)],
        copymagnet: [
          L_("ML_COPY_MAGNETURI"),
          this.torShowMagnetCopy.bind(this)
        ],
    ...
    showProperties: function(k) {
      ...
      if (this.propID != "multi")
        this.request(
          "action=getprops&hash=" + this.propID,
          this.loadProperties
        );
      else this.updateMultiProperties();
    },

    loadProperties: function(json) {
      var props = json.props[0],
        id = this.propID;
      if (!has(this.props, id)) this.props[id] = {};
      for (var k in props) this.props[id][k] = props[k];
    ...
    updateMultiProperties: function() {
      ...
      if (window.utweb === undefined) {
        $("dlgProps-head").set(
          "text",
          "|[" +
            this.trtTable.selectedRows.length +
            " Torrents]| - " +
            L_("DLG_TORRENTPROP_00")
        );
      }
    ...
    updateProperties: function() {
      ...
      for (var k in ids) {
        var dis = props[k] == -1;
        if (k == "dht") dis = !this.settings["dht_per_torrent"];
        ele = $("prop-" + k);
        ele.disabled = dis;
        ele.checked = props[k] == 1;
        $("DLG_TORRENTPROP_1_GEN_" + ids[k])[dis ? "addClass" : "removeClass"](
          "disabled"
        );
      }
      if (window.utweb === undefined) {
        $("dlgProps-head").set(
          "text",
          this.torrents[this.propID][CONST.TORRENT_NAME] +
            " - " +
            L_("DLG_TORRENTPROP_00")
        );
      }
    ...
    setProperties: function() {
      ...
      if (str != "" || window.utweb !== undefined) {
        if (window.getraptor) {
          ...
          this.request(
            "action=setprops&hash=" +
              this.trtTable.selectedRows.join(str + "&hash=") +
              str
          );
        }
      }
    ...
    showDetails: function(id) {
      ...
      switch (
        this.mainTabs.active // TODO: Cleanup... (don't allow direct access to internals)
      ) {
    ...
    clearDetails: function() {
      ["rm", "dl", "ul", "ra", "us", "ds", "se", "pe", "sa", "hs"].each(
        function(id) {
          $(id).set("html", "");
        }
      );
    ...
    updateDetails: function(id) {
      ...
      $("rm").set(
        ...
        d[CONST.TORRENT_ETA] == 0
          ? ""
          : d[CONST.TORRENT_ETA] <= -1
          ? "\u221E"
          : d[CONST.TORRENT_ETA].toTimeDelta()
      );
      $("se").set(
        "html",
        L_("GN_XCONN")
          .replace(/%d/, d[CONST.TORRENT_SEEDS_CONNECTED])
          .replace(/%d/, d[CONST.TORRENT_SEEDS_SWARM])
          .replace(/%d/, "\u00BF?")
      );
      $("pe").set(
        "html",
        L_("GN_XCONN")
          .replace(/%d/, d[CONST.TORRENT_PEERS_CONNECTED])
          .replace(/%d/, d[CONST.TORRENT_PEERS_SWARM])
          .replace(/%d/, "\u00BF?")
      );
    ...
    setLabel: function(param, fn) {
      ...
      var setTorrentLabel = function(torrentHash) {
        var after_update = function() {
          self.request(
            "action=setprops&s=label&v=" + new_label + "&hash=" + torrentHash,
            function() {}
          );
        };
        ...
        self.request(
          "action=setprops&s=label&v=" + "&hash=" + torrentHash,
          after_update
        );
      };
    ...
    addURL: function(param, fn) {
      ...
      if ((val = parseInt(param.dir, 10) || 0)) tail += "&download_dir=" + val;

      if ((val = param.sub || "")) tail += "&path=" + encodeURIComponent(val); // TODO: Sanitize!
    ...
    getPeers: function(id, forceload) {
      ...
      if (
        ...
        !this.peerlist._TIME_ ||
        now - this.peerlist._TIME_ > this.limits.minPeerListCache * 1000
      ) {
        this.request(
          "action=getpeers&hash=" + id,
          function(json) {
            this.peerlist.empty();
    ...
    getFiles: function(id, forceload) {
      ...
      if (
        ...
        !this.filelist._TIME_ ||
        now - this.filelist._TIME_ > this.limits.minFileListCache * 1000
      ) {
        this.request(
          "action=getfiles&hash=" + id,
          function(json) {
            this.filelist.empty();
    ...
    showFileMenu: function(ev) {
      ...
      var prioItems = [
        [
          L_("MF_DONT"),
          this.setPriority.pass([id, CONST.FILEPRIORITY_SKIP], this)
        ],
        [CMENU_SEP],
        [
          L_("MF_LOW"),
          this.setPriority.pass([id, CONST.FILEPRIORITY_LOW], this)
        ],
        [
          L_("MF_NORMAL"),
          this.setPriority.pass([id, CONST.FILEPRIORITY_NORMAL], this)
        ],
        [
          L_("MF_HIGH"),
          this.setPriority.pass([id, CONST.FILEPRIORITY_HIGH], this)
        ]
      ];
    ...
    downloadFiles: function(id) {
      ...
      this.proxyFiles(
        this.torrents[id][CONST.TORRENT_STREAM_ID],
        fileIds,
        false
      );
    },
    ...
    setPriority: function(id, p) {
      ...
      this.request(
        "action=setprio&hash=" + id + "&p=" + p + "&f=" + fileIds.join("&f="),
        function() {
          $each(
            fileIds,
            function(v) {
    ...
    trtColReset: function() {
      var config = {
        colMask: 0,
        colOrder: this.trtColDefs.map(function(item, idx) {
          return idx;
        }),
        colWidth: this.trtColDefs.map(function(item, idx) {
          return item[1];
        })
      };
    ...
    prsColReset: function() {
      var config = {
        colMask: 0,
        colOrder: this.prsColDefs.map(function(item, idx) {
          return idx;
        }),
        colWidth: this.prsColDefs.map(function(item, idx) {
          return item[1];
        })
      };
    ...
    prsDataToRow: function(data) {
      return this.prsColDefs.map(function(item) {
        switch (item[0]) {
          case "ip":
            return (
              ((this.settings["resolve_peerips"] && data[CONST.PEER_REVDNS]) ||
                data[CONST.PEER_IP]) + (data[CONST.PEER_UTP] ? " [uTP]" : "")
            );
    ...
    toggleResolveIP: function() {
      ...
      // TODO: Generalize settings storage requests
      this.request(
        "action=setsetting&s=resolve_peerips&v=" +
          (this.settings["resolve_peerips"] ? 1 : 0),
        function() {
          if (this.torrentID != "") this.getPeers(this.torrentID, true);
        }.bind(this)
      );
    },

    prsSelect: function(ev, id) {
      if (ev.isRightClick()) this.showPeerMenu.delay(0, this, ev);
    },
    ...
    flsColReset: function() {
      var config = {
        colMask: 0,
        colOrder: this.flsColDefs.map(function(item, idx) {
          return idx;
        }),
        colWidth: this.flsColDefs.map(function(item, idx) {
          return item[1];
        })
      };
...
*/
    ...
    advOptChanged: function() {
      ...
      if (optIds.length > 0) {
        ...
        if (id == "bt.transp_disposition") {
          $("enable_bw_management").checked = !!(
            this.getAdvSetting("bt.transp_disposition") & CONST.TRANSDISP_UTP
          );
        }
      }
    },
    ...
    restoreUI: function(bc) {
      if (
        bc != false &&
        !confirm("Are you sure that you want to restore the interface?")
      )
        return;
    ...
    saveConfig: function(async, callback) {
      if (isGuest) return;
      this.request(
        "action=setsetting&s=webui.cookie&v=" + JSON.encode(this.config),
        callback || null,
        async || false
      );
    },
    ...
    toggleCatPanel: function(show) {
      show = show === undefined ? !this.config.showCategories : !!show;
    ...
    toggleDetPanel: function(show) {
      show = show === undefined ? !this.config.showDetails : !!show;
    ...
    toggleDetPanelIcons: function(show) {
      show = show === undefined ? !this.config.showDetailsIcons : !!show;
    ...
    toggleStatusBar: function(show) {
      show = show === undefined ? !this.config.showStatusBar : !!show;
    ...
    toggleToolbar: function(show) {
      show = show === undefined ? !this.config.showToolbar : !!show;
    ...
    tableUseProgressBar: function(enable) {
      ...
      var trtProgCols = this.trtColDefs
        .filter(function(item) {
          return item[2] == TYPE_NUM_PROGRESS;
        })
        .map(function(item) {
          return item[0];
        });
      var prsProgCols = this.prsColDefs
        .filter(function(item) {
          return item[2] == TYPE_NUM_PROGRESS;
        })
        .map(function(item) {
      ...
      var flsProgCols = this.flsColDefs
        .filter(function(item) {
          return item[2] == TYPE_NUM_PROGRESS;
        })
        .map(function(item) {
      ...
      this.trtTable.setConfig({
        colType: trtProgCols.map(progFunc).associate(trtProgCols)
      });
      this.prsTable.setConfig({
        colType: trtProgCols.map(progFunc).associate(prsProgCols)
      });
      this.flsTable.setConfig({
        colType: flsProgCols.map(progFunc).associate(flsProgCols)
      });
    },
    ...
    settingsPaneChange: function(id) {
      ...
      $("dlgSettings-head").innerText =
        document.getElementById("tab_" + id).innerText || L_("ST_CAPT_GENERAL");
    },
    ...
    fdFormatRow: function(values, index) {
      ...
      for (var i = index || 0; i < len; i++) {
        switch (this.fdColDefs[i][0]) {
          ...
          case "codec":
            values[i] =
              CONST.RSSITEMCODECMAP[values[i]] || "UNKNOWN CODEC: " + values[i];
            break;
          ...
          case "episode":
            ...
            values[i] =
              ...
              (episode > 0
                ? episode.pad(2) +
                  (episodeTo > episode ? "-" + episodeTo.pad(2) : "")
                : "");
            break;
          ...
          case "format":
            values[i] =
              CONST.RSSITEMQUALITYMAP[values[i]] ||
              "UNKNOWN QUALITY: " + values[i];
            break;
        }
      }
    ...
    fdColReset: function() {
      var config = {
        colMask: 0,
        colOrder: this.fdColDefs.map(function(item, idx) {
          return idx;
        }),
        colWidth: this.fdColDefs.map(function(item, idx) {
          return item[1];
        })
      };
