...
export var STable = new Class({
  ...
  create: function(id, columns, options) {
    ...
    this.colOrder =
      options.colOrder && options.colOrder.length == this.cols
        ? options.colOrder
        : columns.map(function(item, i) {
            return i;
          });
    this.sIndex = isNaN(options.sIndex)
      ? -1
      : options.sIndex.toInt().limit(-1, this.cols - 1);
    ...
    this.dHead = simpleClone(DIV, false)
      .addClass("stable-head")
      .inject(this.dCont);
    this.dBody = simpleClone(DIV, false)
      .addClass("stable-body")
      .inject(this.dCont);

    this.dHead.addEvents({
      mousedown: function(ev) {
        if (ev.isRightClick()) $me.colMenu.delay(0, $me, ev.page);
      }
    });
    ...
    var nDrag = new Drag(tr, {
      ...
      snap: 1,
      onStart: function() {
        ColumnHandler.start($me, this);
      },
      onDrag: function() {
        ColumnHandler.drag($me, this);
      },
      onComplete: function() {
        ColumnHandler.end($me, this);
      },
    ...
    tr.addEvents({
      mousemove: function(ev) {
        ...
        if (ele.tagName !== "TD") ele = ele.getParent("td");

        if (ele.tagName === "TD") ColumnHandler.check.apply($me, [ev, ele]);
      },
      mousedown: function(ev) {
        var ele = ev.target;
        if (!ele || ele.tagName === "TR") return;
    ...
    for (var i = 0; i < len; i++) {
      ...
      td = simpleClone(TD, false)
        .grab(
          new Element("div", {
            id: this.id + "-head-" + this.colData[i].id,
            text: this.colData[i].text
          })
        )
        .setStyles({
    ...
    this.dPad = simpleClone(DIV, false)
      .addClass("stable-pad")
      .inject(this.dBody);
    ...
    this.colDragObj = simpleClone(DIV, false)
      .addClass("stable-move-header")
      .inject(this.dHead);
    this.colSep = simpleClone(DIV, false)
      .addClass("stable-separator-header")
      .inject(this.dHead);
    ...
    if (this.options.rowsSelectable) {
      this.dBody
        .addEvent("mousedown", function(ev) {
          var ele = ev.target;
          if (ele.get("tag") != "td") ele = ele.getParent("td");
          ...
          if (Browser.ie9 && document.documentMode >= 9) {
            // allow scrollbars to work on IE9
            ContextMenu.hide();
            ev.stopPropagation();
          }
        })
        .addEvent("click", function(ev) {
          ...
          if (!(ele.get("tag") == "td" || ele.getParent("td"))) {
            ...
            if (
              this.clientWidth > ev.page.x - pos.x - this.scrollLeft + 2 &&
              this.clientHeight > ev.page.y - pos.y - this.scrollTop + 2
            ) {
              $me.clearSelection();
        ...
        .addEvent("dblclick", function(ev) {
          ...
          if (ele) {
            ...
            $me.fireEvent(
              "onDblClick",
              ele.getParent().id.replace(idprefix, "")
            );
          }
    ...
    this.infoBar = simpleClone(DIV, false)
      .addClass("stable-infobar")
      .inject(this.dCont);

    if (this.options.rowsSelectable && this.options.rowMultiSelect) {
      this.pageInfo = new Element("span.pageInfo", {
        text: "0 row(s) selected."
      }).inject(this.infoBar);
    }

    if (this.options.refreshable) {
      this.infoBar.grab(
        new Element("div.refreshBtn").grab(
          new Element("span").addEvent("click", function(ev) {
            if ($me.rows) $me.fireEvent("onRefresh");
          })
        )
      );
    }
  ...
  isValidCol: function(iCol, allowUpEdge) {
    return (
      $chk(iCol) &&
      0 <= iCol &&
      (allowUpEdge ? iCol <= this.cols : iCol < this.cols)
    );
  },
  ...
  setColumnPosition: function(iCol, iNew) {
    ...
    var headTarg = this.tHeadCols[iNew],
      headCol = this.tHeadCols[iCol];
    var bodyParent = this.tBody.getElement("colgroup"),
      bodyTarg = bodyParent.childNodes[iNew],
      bodyCol = bodyParent.childNodes[iCol].dispose();
    ...
    function slide(arr) {
      arr.splice(iNew - (iCol < iNew ? 1 : 0), 0, arr.splice(iCol, 1)[0]);
    }
  ...
  setColumnWidth: function(iCol, iWidth) {
    ...
    var offset = (colHead.offsetWidth - parseInt(colHead.style.width, 10)).max(
      0
    );
    ...
    else this.tb.body.setStyle("width", this.tHead.getWidth());
  ...
  setConfig: function(options) {
    ...
    var colHdrIdx = colHdr
      .map(function(item, idx) {
        return idx;
      })
      .associate(
        colHdr.map(function(item) {
          return item.id;
        })
      );
    ...
    if (typeOf(val) === "array") {
      if (val.length == this.cols) {
        this.outOfDOM(
          function() {
            var orderOK = true,
              orderT = Array.clone(val).sort(function(a, b) {
                return a - b;
              });
    ...
    if (!isNaN(val) && val >= 1) {
      var tbBody = this.tb.body,
        tbcCount = tbBody.childNodes.length;
    ...
    if (!isNaN(val) && val != this.options.mode) {
      this.pageChanger.setStyle(
        "display",
        val == MODE_VIRTUAL ? "none" : "block"
      );
  ...
  sort: function(col, shift, revtoggle) {
    ...
    if (col.get("tag") != "td") col = col.getParent("td");
    if (!$chk(col)) return;
    ...
    if (this.sIndex != ind || this.rowCache.length != this.rows) {
      ...
      if (this.isValidCol(this.sIndex))
        this.tHeadCols[this.colOrder[this.sIndex]]
          .removeClass("sorted")
          .getElements("span")
          .destroy();
    }
    ...
    col
      .addClass("sorted")
      .getElement("div")
      .grab(new Element("span." + (this.options.reverse ? "desc" : "asc")));
    this.calcSize();
    ...
    if (!simpleReverse) {
      ...
      switch (this.colHeader[ind].type) {
        ...
        case TYPE_CUSTOM:
          if ($me.sortCustom) {
            comp = function(x, y) {
              return $me.sortCustom(
                ind,
                $me.rowData[x.key].data,
                $me.rowData[y.key].data
              );
            };
            break;
          }
  ...
  sortNumeric: function(x, y) {
    ...
    return r == 0 && this.secIndex != this.sIndex
      ? this.sortSecondary(x, y)
      : r;
  },
  ...
  sortSecondary: function(x, y) {
    ...
    switch (this.colHeader[index].type) {
      ...
      case TYPE_CUSTOM:
        if (this.sortCustom) {
          r = this.sortCustom(
            index,
            this.rowData[x.key].data,
            this.rowData[y.key].data
          );
          break;
        }
    ...
    if (r == 0) r = this.rowData[x.key].index - this.rowData[y.key].index;
    if (this.options.reverse) r = -r;
    if (this.secRev) r = -r;
    return r;
  },

  getActiveRange: function() {
    var max = this.options.maxRows,
      mni = 0,
      mxi = 0;
    if (this.options.mode == MODE_VIRTUAL) {
      mni = (this.activeId.length > 0
        ? Math.floor(this.tBody.offsetTop / this.tb.rowheight) || 0
        : 0
      )
        .min(this.activeId.length - max)
        .max(0);
    } else {
  ...
  refreshRows: function() {
    ...
    var range = this.getActiveRange(),
      count = 0,
      tbc = this.tb.body.childNodes,
      altRows = this.options.alternateRows;
    for (var i = range[0], il = range[1]; i <= il; i++) {
      var id = this.activeId[i],
        rdata = this.rowData[id],
        row = tbc[count];
      if (!(rdata && row)) continue;

      var clsName = "",
        clsChanged = false;
      if (has(this.rowSel, id)) clsName += "selected";
      ...
      if (clsChanged) row.className = clsName.clean();
    ...
    if (!(ev.isRightClick() && has(this.rowSel, id))) {
      ...
      if (multi && ev.shift) {
        ...
        if (has(this.rowSel, id)) {
          ...
          for (
            var i = this.rowSel[id], j = this.selectedRows.length;
            i < j;
            i++
          )
      ...
      if (this.selectedRows.length == 0) this.stSel = null;
  ...
  addRow: function(data, id, icon, hidden, sortin) {
    ...
    if ([sortin, true].pick()) this._insertRow(id);
    this.rows++;
  },
  ...
  _insertRow: function(id, skipOrderCheck) {
    ...
    if (this.isValidCol(sindex)) {
      ...
      if (!skipOrderCheck) {
        var comp,
          from = 0,
          to = this.rowCache.length,
          rIndex = this.rowData[id].index,
          item = {
            key: id,
        ...
        if (rIndex >= 0) {
          if (rIndex != 0 && comp(item, this.rowCache[rIndex - 1]) < 0) {
            ...
            rIndex < this.rowCache.length - 1 &&
            comp(item, this.rowCache[rIndex + 1]) > 0
          ) {
            from = rIndex;
          }
        }
        ...
        if (index != rIndex) {
          ...
          if (rIndex == -1) {
            ...
            var min = index < rIndex ? index : rIndex,
              max = min == index ? rIndex : index;
      ...
      if (this.rowData[id].activeIndex != -1) {
        ...
        if (this.rowData[id].hidden) this.requiresRefresh = true;
      }
      if (!this.rowData[id].hidden) {
        index = this.activeId.binarySearch(id, function(idA, idB) {
          return (
            ($me.rowData[idA].index - $me.rowData[idB].index) *
            ($me.options.reverse ? -1 : 1)
          );
        });
        if (index < 0) index = -(index + 1);
      ...
      if (!this.rowData[id].hidden && this.rowData[id].activeIndex == -1) {
        ...
        var range = this.getActiveRange();
        if (index >= range[0] && index <= range[1]) this.requiresRefresh = true;
      } else if (
        this.rowData[id].hidden &&
  ...
  fillRow: function(row, data) {
    ...
    this.colOrder.each(function(v, k) {
      switch (colh[k].type) {
        case TYPE_NUM_PROGRESS:
          var pcnt = parseFloat(data[k]) || 0,
            pcntp = pcnt.toFixedNR(1) + "%";
          rowc[v].set("html", "").grab(
            simpleClone(DIV, false)
              .addClass("stable-progress")
              .set("text", pcntp)
              .grab(
                simpleClone(DIV, false)
                  .addClass("stable-progress-bar")
                  .setStyle("width", pcntp)
                  .grab(
                    simpleClone(DIV, false)
                      .set("text", pcntp)
                      .setStyle(
                        "width",
                        pcnt ? ((100 * 100) / pcnt).toFixedNR(1) + "%" : 0
                      )
                  )
              )
          );
        ...
        default:
          if (colh[k].icon) {
            rowc[v].set("html", "").grab(
              simpleClone(DIV, false)
                .grab(simpleClone(SPAN, false).set("class", "icon"))
                .appendText(data[k])
            );
          } else {
  ...
  removeRow: function(id) {
    ...
    if (index != -1) {
      ...
      if (this.options.mode == MODE_PAGE) {
        this.pageCount = Math.ceil(this.activeId.length / this.options.maxRows);
        if (this.curPage > this.pageCount) this.curPage--;
      }
      this.requiresRefresh = true;
    }
    if (has(this.rowSel, id)) {
    ...
    if (this.stSel == id) this.stSel = null;
    rd = null;
  ...
  calcSize: function() {
    ...
    this.dBody.setStyles({
      height: (
        this.dCont.clientHeight -
        this.dHead.offsetHeight -
        (showIB ? this.infoBar.offsetHeight : 0)
      ).max(52),
    ...
    if (!this.isResizing) {
      for (var i = 0, j = this.cols; i < j; i++) {
        ...
        var offset = (
          this.tHeadCols[i].offsetWidth -
          parseInt(this.tHeadCols[i].style.width, 10)
        ).max(0);
        this.tHeadCols[i].setStyle(
          "width",
          (
            this.colData[i].width -
            (Browser.chrome || Browser.safari ? 0 : offset)
          ).max(offset)
        );
      }
    }
  ...
  updateCell: function(id, col, data) {
    ...
    if (isSortedCol) this._insertRow(id);
    if (
    ...
    var r = this.tb.body.childNodes[row.rowIndex],
      cell = r.childNodes[this.colOrder[col]],
      fval = this.options.format(Array.clone(data), col);
    if (this.colHeader[col].type == TYPE_NUM_PROGRESS) {
      var pcnt = parseFloat(fval) || 0,
        pcntp = pcnt.toFixedNR(1) + "%";
      cell.set("html", "").grab(
        simpleClone(DIV, false)
          .addClass("stable-progress")
  ...
  resizeTo: function(w, h) {
    var style = {};
    if (typeof w !== "number" || w > 0) style.width = w;
    if (typeof h !== "number" || h > 0) style.height = h;
  ...
  colMenu: function(coords) {
    ...
    this.colHeader.each(function(col, idx) {
      ...
      if (!col.disabled) opts.splice(0, 0, CMENU_CHECK);
  ...
  updatePageMenu: function() {
    ...
    if (this.curPage > 0) this.pagePrev.removeClass("disabled");
    else this.pagePrev.addClass("disabled");

    if (this.curPage < this.pageCount - 1)
      this.pageNext.removeClass("disabled");
    else this.pageNext.addClass("disabled");
    ...
    for (var i = 0; i < this.pageCount; i++) {
      ...
      if (i == this.curPage) this.pageSelect.options[i].selected = true;
    }
  },
  ...
  prevPage: function() {
    if (this.curPage > 0) this.gotoPage(this.curPage - 1);
  },

  nextPage: function() {
    if (this.curPage < this.pageCount - 1) this.gotoPage(this.curPage + 1);
  }
});
...
var ColumnHandler = {
  check: function(ev, cell) {
    ...
    if (x <= 4 && i > 0) {
      // resizing, mouse slightly to right of resize grip
      // Skip hidden columns...
      var j = i - 1;
    ...
    if (x >= cell.offsetWidth - 6) {
      // resizing, mouse slightly to left of resize grip
      this.hotCell = i;
  ...
  start: function initColAct(st, drag) {
    ...
    if (st.hotCell != -1) {
      ...
      // reordering
      var left =
        drag.element.getPosition(st.dCont).x +
        st.dBody.scrollLeft -
        (Browser.firefox && Browser.version >= 3 ? 4 : 0);
      drag.value.now.x = left;
      drag.mouse.pos.x = drag.mouse.start.x - left;
      st.colDragObj.set("html", drag.element.get("text")).setStyles({
        ...
        left: left,
        width:
          drag.element.getStyle("width").toInt() -
          (Browser.firefox || Browser.ie ? 0 : 18), // TODO: Fix up hardcoding!
    ...
    if (st.cancelMove) {
      ...
      var w =
        ...
        (Browser.ie || Browser.firefox
          ? colHead.offsetWidth - parseInt(colHead.style.width, 10)
          : 0);
      drag.limit.x[0] =
        colHead.getPosition(st.dCont).x + st.dBody.getScrollLeft();
      ...
      while (
        i < st.cols &&
        (st.colData[i].disabled ||
          st.tHeadCols[i].getLeft() + st.tHeadCols[i].getWidth() / 2 < x)
      )
        ++i;
      ...
      if (i >= st.cols) {
        ...
        st.colSep.setStyle(
          "left",
          st.tHeadCols[i - 2].offsetLeft + st.tHeadCols[i - 2].getWidth() - 1
        );
      } else {
