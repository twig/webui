...
function setupMouseEvents() {
  ...
  var mouseWhitelist = function(ev) {
    ...
    return (
      ...
      ("input" === tag &&
        !targ.disabled &&
        ["text", "file", "password"].contains(targ.type.toLowerCase())) ||
...
function resizeUI(hDiv, vDiv) {
  ...
  if (!ContextMenu.hidden) ContextMenu.hide();

  var manualH = typeOf(hDiv) == "number",
    manualV = typeOf(vDiv) == "number";

  var size = window.getZoomSize(),
    ww = size.x,
    wh = size.y;
  ...
  if (showDet) {
    ...
    if (vDiv) {
      var dh =
        wh - vDiv - $("mainInfoPane-tabs").getSize().y - (showSB ? 1 : 0) - 14;
...
function setupDetailInfoPaneUI() {
  ...
  $$("#mainInfoPane-generalTab td").addEvent("mousedown", function(ev) {
    ...
    var targ = ev.target;
    if (targ.tagName.toLowerCase() !== "td") targ = targ.getParent("td");
...
function setupRSSDialogs() {
  ...
  var feedSize = $("dlgRSSDownloader-content").getDimensions({
    computeSize: true
  });
  ...
  utWebUI.rssDownloaderTabs = new Tabs("dlgRSSDownloader-tabs", {
    ...
    .draw()
    .show("dlgRSSDownloader-feedsTab");
}
...
function setupSettings() {
  ...
  $("DLG_SETTINGS_4_CONN_04").addEvent("click", function() {
    var v = utWebUI.settings["bind_port"],
      rnd = 0;
    do {
  ...
  $("sched_table")
    .addEvent("change", function() {
      var sv = (utWebUI.settings["sched_table"] || "")
        .pad(7 * 24, "0")
        .substring(0, 7 * 24);
            ...
            (function() {
              ...
              td.set("class", "block mode" + sv.substr(idx, 1)).addEvents({
                  ...
                  if (!active && $("sched_enable").checked) {
                    for (var k = 0; k <= 3; k++) {
                      if (this.hasClass("mode" + k)) {
                        ...
                        sv =
                          sv.substring(0, idx) + mode + sv.substring(idx + 1);
                        break;
                      }
                    }
                    active = true;
                ...
                mouseenter: function() {
                  ...
                  if (
                    $("sched_enable").checked &&
                    active &&
                    !this.hasClass("mode" + mode)
                  ) {
                    this.set("class", "block mode" + mode);
  ...
  utWebUI.advOptTable.create(
    "dlgSettings-advOptList",
    utWebUI.advOptColDefs,
    Object.append(
      {
  ...
  $("DLG_SETTINGS_A_ADVANCED_05").addEvent(
    "click",
    utWebUI.advOptChanged.bind(utWebUI)
  );
  $("dlgSettings-advTrue").addEvent(
    "click",
    utWebUI.advOptChanged.bind(utWebUI)
  );
  $("dlgSettings-advFalse").addEvent(
    "click",
    utWebUI.advOptChanged.bind(utWebUI)
  );
  ...
  $("webui.enable").addEvent(linkedEvent, function() {
    ...
    _link(
      ...
      ["webui.guest"],
      null,
      this.checked && !$("webui.enable_guest").checked
    );
    _link(
      this,
      0,
      ["webui.port"],
      null,
      this.checked && !$("webui.enable_listen").checked
    );
  });
...
function _unhideSetting(obj) {
  Array.from(obj).each(function(ele) {
    ...
    while (
      ele &&
      !ele.hasClass("settings-pane") &&
      ele.getStyle("display") === "none"
    ) {
      ele.show();
      ele = ele.getParent();
    }
...
function loadSettingStrings() {
  ...
  if (utWebUI.advOptTable.tb.body) {
  ...
  _loadComboboxStrings(
    "gui.dblclick_seed",
    L_("ST_CBO_UI_DBLCLK_TOR").split("||"),
    utWebUI.settings["gui.dblclick_seed"]
  );
  _loadComboboxStrings(
    "gui.dblclick_dl",
    L_("ST_CBO_UI_DBLCLK_TOR").split("||"),
    utWebUI.settings["gui.dblclick_dl"]
  );
  _loadComboboxStrings(
    "encryption_mode",
    L_("ST_CBO_ENCRYPTIONS").split("||"),
    utWebUI.settings["encryption_mode"]
  );
  _loadComboboxStrings(
    "proxy.type",
    L_("ST_CBO_PROXY").split("||"),
    utWebUI.settings["proxy.type"]
  );
  _loadComboboxStrings(
    "multi_day_transfer_mode",
    L_("ST_CBO_TCAP_MODES").split("||"),
    utWebUI.settings["multi_day_transfer_mode"]
  );
  _loadComboboxStrings(
    "multi_day_transfer_limit_unit",
    L_("ST_CBO_TCAP_UNITS").split("||"),
    utWebUI.settings["multi_day_transfer_limit_unit"]
  );
  _loadComboboxStrings(
    "multi_day_transfer_limit_span",
    L_("ST_CBO_TCAP_PERIODS").split("||"),
    utWebUI.settings["multi_day_transfer_limit_span"]
  );
...
function _loadComboboxStrings(id, vals, def) {
  try {
    ...
    console.log(
      "Error attempting to assign values to combobox with id='" + id + "'... "
    );
