      ...
      switch (item[0]) {
        ...
        case CMENU_CHILD:
          ...
          li.adopt(div).addStopEvents({
            mouseenter: function() {
              ContextMenu.show(this.getCoordinates(), div);
            },
            mouseleave: function() {
              ContextMenu.hide(div);
            }
          });
  ...
  scroll: function(ev) {
    ...
    if (ulc) {
      ...
      var scrollMax =
        dpos.height -
        ul.getHeight() -
        ul.getDimensions({ computeSize: true })["padding-top"];
      ...
      if (ev.page.y < dpos.top + ulch) {
        ...
        top = (
          (scrollMax * (ev.page.y - dpos.top - ulch)) /
          (dpos.height - ulch - ulch)
        )
          .toFixed(0)
          .toInt();
      }
